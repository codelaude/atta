...
- **3.0.0** (2026-03-14): Enforcement Infrastructure — dir restructure (team/local split), 17 skills renamed to `atta-*` namespace + 2 hidden skills, 4 core + 4 optional agents, new architect agent, rich agent frontmatter (14 fields, cross-tool enforcement), path-scoped rules (5 adapter formats), data-driven enforcement hooks, model targeting (`model-registry.json`), rules-aware CI review, init absorption, `/atta-migrate` rewrite, hook hardening (28 tests), confidence-scored reviews (`--strict`/`--quiet`), compound + semantic detection, agent enforcement hooks, progressive disclosure for skills, agent negative triggers, KISS review system (simplicity gate in `/atta-ship` + Simplicity & Design domain in `/atta-review`)
