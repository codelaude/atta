...
6b. **Codex-only: also write a TOML agent config file** (if running on Codex — detected by `.agents/skills/` existing):

   Codex CLI reads `config_file` entries in `.codex/config.toml` as TOML. Markdown files with YAML frontmatter (`---`) are not valid TOML and will fail to parse. For each agent written in step 6, also:

   a. **Compute the TOML path** — mirror the markdown path but under `.codex/agents/` with a `.toml` extension:
      - Root/core agents: `.codex/agents/{name}.toml`
      - Coordinators: `.codex/agents/coordinators/{name}.toml`
      - Specialists: `.codex/agents/specialists/{name}.toml`

   b. **Build TOML content** — embed the agent body (everything after the `---` frontmatter fence) as a `system_prompt` multiline basic string. Escape the body:
      - Replace `\` with `\\` (must be done first)
      - Replace `"""` with `\"\"\"` (to avoid closing the TOML multiline string prematurely)
      ```toml
   ...
   c. **Add entry to `.codex/config.toml`** — append a new `[agents.{safe_name}]` section:
      ```toml
      [agents.{name}]
      description = "{DESCRIPTION_TOML_ESCAPED}"
      config_file = "agents/{subdir}/{name}.toml"
      ```
      where:
      - `{safe_name}` is the agent name, quoted in double-quotes if it contains chars outside `[A-Za-z0-9_-]`
      - `{DESCRIPTION_TOML_ESCAPED}` has `\`, `"`, `\n`, `\r`, `\t` replaced with their TOML escape sequences
      - `config_file` is always relative to `.codex/` (e.g., `"agents/specialists/vue.toml"`)
