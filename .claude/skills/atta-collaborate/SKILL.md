---
name: atta-collaborate
description: Multi-agent collaboration for cross-domain code review, architecture feedback, and design decisions. Invokes 2-4 specialist agents in parallel and synthesizes their findings with automated conflict detection. Does NOT handle single-agent tasks (use /atta-agent) or simple reviews (use /atta-review).
model: opus
...
```
/atta-collaborate                                          # Review mode on git diff (auto-selects agents)
/atta-collaborate src/components/UserProfile.tsx           # Review specific file
...
/atta-collaborate --quick                                  # Critical + High findings only
/atta-collaborate --mode feedback                          # Architecture feedback (no file-level findings)
/atta-collaborate --mode decision "REST vs GraphQL?"       # Decision analysis with agent perspectives
/atta-collaborate --skip-synthesis                         # Raw agent outputs without conflict detection
```
...
7. **Next Steps** — follow-up suggestions (`/atta-review`, `/atta-agent {id}`, `/atta-preflight`)
...
| No agents available | Run `/atta`, use `/atta-review`, or specify `--agents code-reviewer,qa-validator` |
| Single agent only | Add second agent explicitly, broaden scope, or use `/atta-agent` instead |
| Agent failure | Continue with remaining agents, retry failed agent, or use `/atta-agent` standalone |
| All agents failed | Retry, reduce agent count, or fall back to `/atta-review` |
...
- `/atta-review` — Single-agent code review
- `/atta-security-audit` — Deep security scan (OWASP Top 10)
- `/atta-preflight` — Full pre-PR validation
- `/atta-agent {id}` — Single specialist analysis
- `/atta-team-lead` — Task decomposition
