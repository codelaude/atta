...
3. **Determine the target agent** (who made the suggestion being corrected):
   ...
   - If correcting output from a recent `/atta-agent` or `/atta-collaborate` invocation, use the active agent ID
   - If correcting output from `/atta-review`, use `code-reviewer`
...
6. If the pattern count reaches its threshold, inform the user:
   > "Pattern '{key}' has been corrected {N} times and is ready for promotion. Run `/atta-patterns suggest` to see details."
...
```
/atta-librarian Remember to always use aria-live in parent components, not children
/atta-librarian Capture that we decided to use server-side caching for API responses
/atta-librarian From now on, all modals should trap focus on open
/atta-librarian --migrate
```
...
- **Knowledge Files**:
  - Pattern files in `.atta/team/patterns/`
