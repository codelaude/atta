...
```
/atta-optimize <prompt>                          # Enrich for use in the current session
/atta-optimize --rephrase <prompt>              # Rephrase a prompt that didn't get a good result
/atta-optimize --target codex <prompt>           # Optimize for Codex
/atta-optimize --target copilot <prompt>         # Optimize for Copilot
/atta-optimize --target chatgpt <prompt>         # Optimize for ChatGPT
/atta-optimize --target gemini <prompt>          # Optimize for Gemini
/atta-optimize --target claude <prompt>          # Optimize for another Claude instance
```
...
- `/atta` — Generates the project context that `/atta-optimize` reads
- `/atta-profile` — Sets preferences that `/atta-optimize` injects into prompts
- `/atta-agent prompt-engineer` — The underlying agent (invoked directly for custom workflows)
