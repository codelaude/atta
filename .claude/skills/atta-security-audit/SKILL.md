...
description: Security audit scanning for vulnerabilities, secrets, and insecure patterns. Covers OWASP Top 10 (2025), dependency vulnerabilities, and hardcoded credentials. Does NOT fix vulnerabilities or perform code quality review (use /atta-review).
...
```
/atta-security-audit                      # Full security audit on changed files
/atta-security-audit src/api/             # Audit specific folder
/atta-security-audit auth.ts              # Audit specific file
/atta-security-audit --dependencies       # Dependency vulnerability scan only
/atta-security-audit --secrets            # Secrets/credentials scan only
/atta-security-audit --quick              # Critical findings only (fast mode)
```
...
```
# AWS Secret Key — context keyword must appear within 20 chars of the value
(aws_secret_access_key|AWS_SECRET_ACCESS_KEY).{0,20}['":=]\s*[A-Za-z0-9/+=]{40}

# Generic API Key — context keyword must appear within 40 chars of the value
(api[_-]?key|token|secret).{0,40}['"][A-Za-z0-9_-]{24,}['"]

# Database URL — credentials embedded in connection string
...
**Context validation**: The `.{0,N}` spans in the AWS and API key patterns enforce that the security keyword and the secret value appear on the same line within N characters. For matches without built-in context (Database URL, Password), verify the match is not in a test file, `.env.example`, or documentation before flagging.
...
After `/atta-security-audit`:
- Run `/atta-review` for code quality review (framework patterns, performance, bug/logic)
- Run `/atta-preflight` for complete pre-PR validation (includes this scan automatically)
- Use `/atta-agent security-specialist` for interactive security guidance
...
2. Provide a file/folder target explicitly (`/atta-security-audit path/to/file`)
...
3. Run `/atta-security-audit --secrets` for secrets scan only (no deps needed)
```
