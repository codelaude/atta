...
```
/atta-test                     # Run unit/integration tests (auto-detect framework)
/atta-test --e2e               # Run E2E tests only
/atta-test --coverage          # Run tests with coverage analysis
/atta-test --watch             # Run tests in watch mode
/atta-test --e2e --coverage    # E2E tests with coverage
/atta-test <path>              # Run tests for a specific file or directory
```
...
- `/atta-preflight` — Includes test execution as part of pre-PR validation
- `/atta-review` — Code review (pair with test results)
- `/atta-agent jest` / `/atta-agent e2e` — Testing specialist guidance
