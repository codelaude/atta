...
```bash
/atta-update check --from ./.claude_staging/.claude
/atta-update pull --dry-run --from ./.claude_staging/.claude
/atta-update pull --from ./.claude_staging/.claude
/atta-update pull --interactive --from ./.claude_staging/.claude
/atta-update pull --from ./.claude_staging/.claude --mode upgrade
/atta-update pull --from ./.claude_staging/.claude --mode migration
/atta-update pull --from ./.claude_staging/.claude --mode migration-bootstrap
/atta-update rollback
/atta-update --history
```
...
When user runs `/atta-update check`:
...
Show per-tier summary: Tier 1 files to replace (with counts), Tier 2 files needing merge (with customization details), Tier 3 preserved files, generated files (suggest `/atta --rescan`). End with next steps: `/atta-update pull` or `--interactive`.

---

## Phase 3: Apply Updates (`/atta-update pull`)
...
> **`## Project Rules` sections** are user-owned content promoted from scoped directives via `/atta-patterns promote --directives`. They must be extracted before update and reapplied after, same as personality names and added rules. Never overwrite or remove them.
...
Show: version transition, counts (updated/merged/preserved), backup location, next steps (`/atta --rescan`, `git diff .claude/`, `/atta-update rollback`).

---

## Phase 5: Rollback (`/atta-update rollback`)
...
## Phase 6: Update History (`/atta-update --history`)
...
- `## Project Rules` — promoted directives from `/atta-patterns promote --directives`
