...
import { copyAgentFiles, copyBootstrap, copySharedContent, rewriteSkillBody, filterSkillFrontmatter, CODEX_SKILL_FIELDS, checkSkillConflicts, createMemoryDirectory, listAgentDefs, generateHooks, parseAgentFrontmatter } from './shared.js';
...
/**
 * Write a per-agent TOML config file for Codex CLI.
 * Reads the source .md agent definition, extracts the body (after frontmatter),
 * rewrites Claude-specific paths/commands, and embeds the result as a
 * TOML `system_prompt` multiline basic string.
 *
 * @param {string} srcPath - Source .md agent file path
 * @param {string} destPath - Destination .toml file path
 * @param {object} rewriteConfig - Config for rewriteSkillBody (paths, commandMap, etc.)
 */
function writeAgentToml(srcPath, destPath, rewriteConfig) {
  const content = readFileSync(srcPath, 'utf-8');
  const { body } = parseAgentFrontmatter(content);
  const rewrittenBody = rewriteSkillBody(body, rewriteConfig).trim();
...
export function install(claudeRoot, attaRoot, targetDir, options = {}) {
  ...
  // and per-agent .codex/agents/{name}.toml TOML config files.
  // Codex CLI reads `config_file` as TOML — .md files fail parsing because
  // YAML frontmatter (---) is not valid TOML. Each agent gets a dedicated
  // .toml file with a `system_prompt` multiline string containing the agent body.
  ...
  if (agentDefs.length > 0) {
    ...
    // Create .codex/agents/ for per-agent TOML files (one per agent)
    ...
    const tomlLines = [
      ...
      '# Generated by Atta framework — do not edit manually.',
      '# See .agents/agents/ for full agent instructions (human-readable markdown).',
      '',
    ];

    let tomlAgentCount = 0;
    for (const agent of agentDefs) {
      const baseName = agent.fileName.replace(/\.md$/, '');
      const srcPath = join(claudeRoot, 'agents', agent.fileName);
      const destTomlPath = join(codexAgentsDir, `${baseName}.toml`);

      // Write per-agent TOML file with system_prompt
      writeAgentToml(srcPath, destTomlPath, codexRewriteConfig);
      tomlAgentCount++;
      ...
      // config_file is relative to .codex/ — points to the TOML agent file (not the .md source)
      tomlLines.push(`config_file = "agents/${baseName}.toml"`);
      tomlLines.push('');
    }
    ...
    results.files++;
    results.files += tomlAgentCount;

    if (!options.quiet) {
      console.log(
        `  ${pc.green('✓')} .codex/agents/ (${tomlAgentCount} agent TOML files)`
      );
      console.log(
