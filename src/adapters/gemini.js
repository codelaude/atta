 ...
 * Gemini uses snake_case names: read_file, edit_file, grep_search, etc.
 *
 * @param {string[]|string} tools - Claude Code tool names
 * @returns {string[]} Gemini tool names
 */
function mapToolsToGemini(tools) {
  // Verified against gemini-cli docs (docs/tools/file-system.md, docs/tools/shell.md)
  const CC_TO_GEMINI = {
    Read: 'read_file',
    Edit: 'replace',       // Gemini's edit tool is registered as 'replace'
    Write: 'write_file',
    Grep: 'grep_search',
    Glob: 'glob',          // FindFiles tool, not list_directory
    Bash: 'run_shell_command',
    // Agent: no generic invoke_agent — subagents are referenced by their name
  };

  const list = Array.isArray(tools) ? tools : tools.split(/,\s*/);
  const mapped = new Set();
  for (const tool of list) {
    const geminiName = CC_TO_GEMINI[tool.trim()];
    if (geminiName) mapped.add(geminiName);
  }
  return [...mapped];
}
