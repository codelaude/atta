 ...
 * reads project context files (.atta/project/) and pattern files (.atta/team/patterns/)
 ...
 * (the workflow always reads the latest .atta/project/ and .atta/team/patterns/ files, so this is rarely needed).
 */
export function install(claudeRoot, attaRoot, targetDir, options = {}) {
  ...
  const suppressionsPath = join(teamDir, 'ci-suppressions.md');
  if (!existsSync(suppressionsPath)) {
    ...
    if (!options.quiet) {
      console.log(`  ${pc.green('✓')} .atta/team/ci-suppressions.md`);
    }
  }

  // Generate review guidance file for CI context
  // The CI prompt reads .atta/team/ files — place review guidance there
  const reviewGuidancePath = join(teamDir, 'review-guidance.md');
  mkdirSync(teamDir, { recursive: true });
  const reviewRules = generateReviewRules(attaRoot, options.detectedTechs);
  const ciReview = formatCIReview(reviewRules);
  writeFileSync(reviewGuidancePath, `# Review Guidance\n\n> Stack-specific review rules generated by Atta.\n> The CI reviewer reads this file automatically.\n${ciReview}`);
  results.files++;
 ...
 * Instructs the CI model to read .atta/team/ files before reviewing.
 ...
 *
 * The prompt instructs the CI model to read .atta/team/ files before
