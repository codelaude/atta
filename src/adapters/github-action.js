...
export function install(claudeRoot, attaRoot, targetDir, options = {}) {
  ...
  // The CI prompt already reads .atta/knowledge/ files — place review guidance there
  const reviewGuidancePath = join(knowledgeDir, 'review-guidance.md');
  if (!existsSync(reviewGuidancePath)) {
    mkdirSync(knowledgeDir, { recursive: true });
    ...
    if (!options.quiet) {
      console.log(`  ${pc.green('✓')} .atta/knowledge/review-guidance.md (CI review context)`);
    }
  }
