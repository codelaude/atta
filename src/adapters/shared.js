...
function generateCopilotHooks(detectedTechs) {
  ...
  hooks[HOOK_EVENT_MAP.preToolUse.copilot] = [
    ...
    },
    {
      ...
      timeoutSec: 5,
      comment: 'Advise switching to a cheaper model for lightweight skills',
    },
  ];
...
/**
 * Model gate hook script — advisory only. Warns on stderr when a light-tier skill
 * runs on a potentially expensive model, suggesting a cheaper alternative.
 * Always exits 0 (does not block). Reads skill tier from .atta/team/model-registry.json.
 * The --bypass flag suppresses the warning entirely.
 */
export const MODEL_GATE_SCRIPT = `#!/bin/bash
# model-gate.sh — Advisory: suggest cheaper models for lightweight skills
# Generated by Atta. Reads model-registry.json for tier + tool-specific model names.
# Emits a stderr warning for light-tier skills. Always exits 0 (does not block).
# User can suppress with --bypass flag.
set -euo pipefail
...
# Look up skill tier and tool-specific model name from registry
...
# Advisory only: we cannot detect the current model, so we warn but don't block.
# Blocking unconditionally would make light-tier skills unusable without --bypass.
# Claude Code handles routing natively via model: frontmatter — this hook is for
# Copilot/Cursor/Gemini where users must switch models manually.
if [ "\$TIER" = "light" ]; then
