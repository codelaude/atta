...
import { copyAgentFiles, rewriteSkillBody, filterSkillFrontmatter, COPILOT_SKILL_FIELDS, CURSOR_SKILL_FIELDS, CODEX_SKILL_FIELDS, generateHooksConfig, writeHookScripts, mapToolsToCopilot } from '../adapters/shared.js';
 ...
 * plus a compatibility .codex/config.toml placeholder.
 *
 * Output structure:
 *   skills/<name>/SKILL.md (rewritten with $prefix)
 *   agents/<name>.md (frontmatter filtered to name+description, body rewritten)
 *   .codex/config.toml (compatibility placeholder, no native [agents.*] registration)
 *   AGENTS.md
 *   README.md
 */
function generateCodexPlugin(claudeRoot, attaRoot, outputBase) {
  ...
  // 3. Generate a compatibility placeholder that clears stale Atta-generated
  // agent role registrations from older installs.
  const codexConfigDir = join(pluginDir, '.codex');
  mkdirSync(codexConfigDir, { recursive: true });
  writeAndSync(join(codexConfigDir, 'config.toml'), [
  ...
  files++;
  summary.push('.codex/config.toml (compatibility placeholder)');
