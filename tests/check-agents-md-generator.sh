...
LINE_COUNT=$(wc -l < "$TMPDIR/AGENTS.md" | tr -d ' ')
...
for section in "Project Overview" "Build" "Code Style" "Architecture"; do
  if ! grep -qi "$section" "$TMPDIR/AGENTS.md"; then
...
AGENT_ROWS=$(grep -c '^\| ' "$TMPDIR/AGENTS.md" 2>/dev/null || true)
...
if [ -f "$GOLDEN" ]; then
  if ! diff -q "$GOLDEN" "$TMPDIR/AGENTS.md" >/dev/null 2>&1; then
    echo "WARN: AGENTS.md output differs from golden snapshot"
    echo "  Update golden: cp $TMPDIR/AGENTS.md $GOLDEN"
